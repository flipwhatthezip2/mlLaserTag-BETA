...
public class LaserTag implements CommandExecutor {
    ...
    public void trySelectingGun(CommandSender sender, String[] args){
        ...
        if (!(sender instanceof Player)) {
            sender.sendMessage(GameAPI.infoMessage + "Only players can select guns");
            return;
        }
        ...
        if (Arena.getArena(core, player) != null){
            player.sendMessage(GameAPI.infoMessage + "You can't select a gun while in an arena");
            return;
        }

        if (!core.getGunsFile().contains(args[1])){
            player.sendMessage(GameAPI.infoMessage + "Gun does not exist");
            return;
        }
    ...
    public void trySpawningGunSelector(CommandSender sender, String[] args){
        if (!sender.isOp()){
            sender.sendMessage(GameAPI.infoMessage + "You don't have permission to do this");
            return;
        }

        if (!(sender instanceof Player)){
            sender.sendMessage(GameAPI.infoMessage + "Only players can spawn GunSelector NPC's");
        }
    ...
    public void trySpawningArenaSelector(CommandSender sender, String[] args){
        ...
        if (!(sender instanceof Player)){
            sender.sendMessage(GameAPI.infoMessage + "Only players can spawn ArenaSelector NPC's");
        }
    ...
    public void tryJoining(CommandSender sender, String[] args){
        ...
        if (!(sender instanceof Player)){
            sender.sendMessage(GameAPI.infoMessage + "Only players can join arenas");
            return;
        }

        if (args.length != 2){
            sender.sendMessage(GameAPI.infoMessage + "Invalid argument amount");
            return;
        }

        if (!arenasFile.getArenaNames().contains(args[1])){
            sender.sendMessage(GameAPI.infoMessage + "Specified arena does not exist");
            return;
        }

        if (Arena.getArena(core, (Player) sender) != null){
            sender.sendMessage(GameAPI.infoMessage + "You're already in an arena");
            return;
        }

        if (!Arena.getArena(core, args[1]).getCanJoin()){
            sender.sendMessage(GameAPI.infoMessage + "Specified arena is currently not joinable");
            return;
        }
    ...
    public void tryLeaving(CommandSender sender, String[] args){
        ...
        if (!(sender instanceof Player)){
            sender.sendMessage(GameAPI.infoMessage + "Only players can leave arenas");
            return;
        }
        ...
        if (Arena.getArena(core, player) == null){
            sender.sendMessage(GameAPI.infoMessage + "You're not in an arena");
            return;
        }
    ...
    public void lasertaggun(CommandSender sender, String[] args){
        ...
        if (!sender.isOp()){
            sender.sendMessage(GameAPI.infoMessage + "You don't have permission to execute this command");
            return;
        }
        ...
        if (args.length < 2){
            sender.sendMessage(GameAPI.infoMessage + "Invaild argument ammount.");
            return;
        }
        ...
        if (args[1].equalsIgnoreCase("create")){
            ...
            if (core.getGunsFile().contains(args[2])){
                sender.sendMessage(GameAPI.infoMessage + "Gun with this name already exists");
                return;
            }
            ...
            try {
                ...
                sender.sendMessage(GameAPI.infoMessage + "Some values are not valid integers");
                return;
            }
            ...
            sender.sendMessage(GameAPI.infoMessage + "Successfully created gun");
            ...
            if (!core.getGunsFile().contains(args[2])){
                sender.sendMessage(GameAPI.infoMessage + "Gun does not exist");
                return;
            }

            core.getGunsFile().deleteGun(args[2]);
            sender.sendMessage(GameAPI.infoMessage + "Successfully deleted gun");
            Bukkit.getPluginManager().callEvent(new GunStatUpdate(Gun.getGun(args[2])));
            ...
            if (!isValidStat){
                sender.sendMessage(GameAPI.infoMessage + "Invalid GunStat. GunStats are CoolDown, Range, Damage, and Zoom");
                return;
            }
            ...
            try {
                ...
                sender.sendMessage(GameAPI.infoMessage + "4th argument is an invalid integer");
                return;
            }

            core.getGunsFile().setGunStat(args[2], gunStat, Integer.parseInt(args[4]));
            sender.sendMessage(GameAPI.infoMessage + "Successfully set GunStat");
            Bukkit.getPluginManager().callEvent(new GunStatUpdate(Gun.getGun(args[2])));
            ...
            sender.sendMessage(GameAPI.infoMessage + "Successfully renamed gun");
            Bukkit.getPluginManager().callEvent(new GunStatUpdate(Gun.getGun(args[2])));
    ...
    public void lasertag(CommandSender sender){
        sender.sendMessage(GameAPI.infoMessage + "Currently running §cLaser Tag §lBETA");
        sender.sendMessage(GameAPI.infoMessage + "Developed by: §cFlipwhatthezip2 §eand §cReadySetPawn");
        sender.sendMessage(GameAPI.infoMessage + "For a list of commands type: §c/lasertag §lhelp");
    }
    public void lasertagarena(CommandSender sender, String[] args){
        ...
        if (args.length == 1) {
            sender.sendMessage(GameAPI.infoMessage + "Commands for §c§lLASERTAG ARENAS§e:");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena create [arena] [world] §8- §7Creates arena.");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena delete [arena] §8- §7Deletes arena.");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena maxPlayer [arena] [maxPlayers] §8- §7Sets max players.");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena minPlayer [arena] [minPlayers] §8- §7Sets min players.");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena setYellowSpawn [arena] §8- §7Sets spawn.");
            sender.sendMessage(GameAPI.infoMessage + "/lasertag arena setGreenSpawn [arena] §8- §7Sets spawn.");
            return;
        }
        ...
        if (args[1].equalsIgnoreCase("create")){
            ...
            if (arenasFile.getArenaNames().contains(args[2])){
                sender.sendMessage(GameAPI.infoMessage + "Invaild arena name. Is it created?");
                return;
            }

            if (Bukkit.getWorld(args[2]) == null){
                sender.sendMessage(GameAPI.infoMessage + "Invaild world name. Make sure you input the correct world name.");
                return;
            }
            ...
            sender.sendMessage(GameAPI.infoMessage + "Successfully deleted the arena: §a§l" + args[2] + "§7!");
        }
            ...
            try {
                ...
                sender.sendMessage(GameAPI.infoMessage + "4th argument is an invaild integer.");
                return;
            }
            ...
            if (args[1].equalsIgnoreCase("minPlayers")){
                ...
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §a§lMIN §7players!");
            }

            if (args[1].equalsIgnoreCase("maxPlayers")){
                ap.setMaximumPlayers(Integer.parseInt(args[3]));
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §a§lMAX §7players!");
            }
            ...
            if (!(sender instanceof Player)){
                sender.sendMessage(GameAPI.infoMessage + "Spawn setting failed. You must be ingame to set a spawn!");
                return;
            }
            ...
            if (args[1].equalsIgnoreCase("setYellowSpawn")){
                ...
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §e§lYELLOW §7spawn!");
            }

            if (args[1].equalsIgnoreCase("setGreenSpawn")){
                core.getArenasFile().setGreenSpawn(args[2], player.getLocation());
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §2§lGREEN §7spawn!");
            }
        }
            ...
            if (!(sender instanceof Player)){
                sender.sendMessage(GameAPI.infoMessage + "Beacon setting failed. You must be ingame to set a spawn!");
                return;
            }
            ...
            if (args[1].equalsIgnoreCase("setYellowBeacon")){
                ...
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §e§lYELLOW §7beacon!");
            }

            if (args[1].equalsIgnoreCase("setGreenBeacon")){
                core.getArenasFile().setGreenBeacon(args[2], player.getLocation());
                sender.sendMessage(GameAPI.infoMessage + "Successfully set the §2§lGREEN §7beacon!");
            }
        }

    }
    public void lasertaghelp(CommandSender sender){
        sender.sendMessage(GameAPI.infoMessage + "Commmands for §c§lLASERTAG§e:");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag debug §8- §7Displays the current builds debug info.");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag info §8- §7Displays the plugin info.");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag arena §8- §7Displays all arenas commands.");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag game §8- §7Displays the current games info.");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag start §8- §7Force starts the current game.");
        sender.sendMessage(GameAPI.infoMessage + "/lasertag stop §8- §7Force stops the current game.");
    }
}
